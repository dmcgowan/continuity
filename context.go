...
func (c *context) checkoutFile(fp string, rf RegularFile, flag int) error {
	if c.provider == nil {
	...
	f, err := os.OpenFile(fp, flag, rf.Mode())
	if err != nil {
...
func (c *context) Apply(resource Resource) error {
	...
	case RegularFile:
		if fi == nil {
			if err := c.checkoutFile(fp, r, os.O_WRONLY|os.O_CREATE|os.O_TRUNC); err != nil {
			...
			if fi.Size() != r.Size() {
				if err := c.checkoutFile(fp, r, os.O_WRONLY|os.O_TRUNC); err != nil {
					return fmt.Errorf("error checking out file %q: %v", resource.Path(), err)
				}
			} else {
